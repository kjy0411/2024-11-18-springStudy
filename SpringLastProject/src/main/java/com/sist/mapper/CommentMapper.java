...
public interface CommentMapper {
	// 목록
	@Select("SELECT no,cno,type,userid,username,sex,msg,group_id,group_step,TO_CHAR(regdate,'YYYY-MM-DD HH24:MI:SS') as dbday,num "
			...
			+ "FROM busanReply WHERE cno=#{cno} AND type=#{type} ORDER BY group_id DESC,group_step ASC)) "
	...
	@Insert("INSERT INTO busanReply(no,cno,type,userid,username,sex,msg,group_id,group_step) "
			+ "VALUES((SELECT NVL(MAX(no)+1,1) FROM busanReply),#{cno},#{type},#{userid},#{username},#{sex},#{msg},(SELECT NVL(MAX(group_id)+1,1) FROM busanReply),#{group_step})")
	...
	// 대댓글
	@Select("SELECT group_id,group_step FROM busanReply WHERE no=#{no}")
	public CommentVO commentParentInfoData(int no);
	@Update("UPDATE busanReply SET group_step=group_step+1 WHERE group_id=#{group_id} AND group_step>#{group_step}")
	public void commentGroupStepIncrement(CommentVO vo);
	@Insert("INSERT INTO busanReply(no,cno,type,userid,username,sex,msg,group_id,group_step) "
			+ "VALUES((SELECT NVL(MAX(no)+1,1) FROM busanReply),#{cno},#{type},#{userid},#{username},#{sex},#{msg},#{group_id},#{group_step})")
	public void commentReplyInsert(CommentVO vo);
}
